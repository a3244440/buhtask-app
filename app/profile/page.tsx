...
interface Profile {
  ...
  role: string; bio: string; avatar_url: string; experience_years: number;
  min_price: number; specialization: string[];
}
...
export default function ProfilePage() {
  ...
  const [profile, setProfile] = useState<Profile>({
    id: '', email: '', full_name: '', phone: '', city: 'Астана',
    role: 'client', bio: '', avatar_url: '', experience_years: 0,
    min_price: 0, specialization: [],
  });
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError('Ошибка загрузки фото: ' + (err.message || 'Попробуйте снова'));
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error: e } = await supabase.from('profiles').update(update).eq('id', profile.id);
      if (e) throw e;
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        <button onClick={() => router.push(dashHref)} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-800 mb-6 transition-colors">
          <ArrowLeft className="w-4 h-4" /> Назад в кабинет
        </button>

        {/* Avatar */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5">
          <h2 className="font-semibold text-gray-900 mb-5">Фото профиля</h2>
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1.5">JPG, PNG до 5MB</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5 space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              ...
              {profile.role === 'accountant' ? 'ФИО' : 'Имя / Название компании'}
            </label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              <Phone className="w-4 h-4 inline mr-1.5 text-gray-400" />Телефон
            </label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">
              <MapPin className="w-4 h-4 inline mr-1.5 text-gray-400" />Город
            </label>
        ...
        {profile.role === 'accountant' && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5 space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                <FileText className="w-4 h-4 inline mr-1.5 text-gray-400" />О себе
              </label>
              <textarea rows={4} value={profile.bio} onChange={e => setProfile(p => ({ ...p, bio: e.target.value }))}
                placeholder="Расскажите о своём опыте, специализации и преимуществах..."
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  <Briefcase className="w-4 h-4 inline mr-1.5 text-gray-400" />Опыт (лет)
                </label>
                <input type="number" min="0" max="50" value={profile.experience_years}
              ...
              <div>
                ...
                <input type="number" min="0" value={profile.min_price}
                  onChange={e => setProfile(p => ({ ...p, min_price: parseInt(e.target.value) || 0 }))}
        ...
        {success && <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-sm mb-4 flex items-center gap-2">
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
          {success}
        </div>}

        <button onClick={handleSave} disabled={saving}
          className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white py-3.5 rounded-xl font-semibold transition-colors text-sm">
