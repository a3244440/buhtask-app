...
export default function EditDocPage() {
  ...
  const init = async () => {
    ...
    const { data: doc } = await supabase.from('documents').select('*').eq('id', docId).eq('owner_id', user.id).maybeSingle();
    if (!doc) { router.push('/documents'); return; }
    ...
    setCounterpartyId(doc.counterparty_id || '');
    setNumber(doc.number || '');
    setDocDate(doc.doc_date ? new Date(doc.doc_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="lg:pl-60">
        <DashboardHeader title={`${t('nd.editTitle')}: ${TYPE_LABEL}`} />
        <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
          <button onClick={() => router.push(`/documents/${docId}`)} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 mb-5">
            <ArrowLeft className="w-4 h-4" /> {t('nd.toDocuments')}
          </button>
          <h1 className="text-xl font-bold text-gray-900 mb-1">{t('nd.editTitle')}: {TYPE_LABEL}</h1>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5 mt-4">
            ...
            <button onClick={save} disabled={saving} className="w-full flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white py-3 rounded-xl font-semibold text-sm transition-colors">
              <Save className="w-4 h-4" /> {saving ? t('doc.saving') : t('nd.saveChanges')}
            </button>
          </div>
        </main>
      </div>
