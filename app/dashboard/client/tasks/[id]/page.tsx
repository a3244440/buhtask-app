    ...
    <div className="min-h-screen bg-[#F8FAFC]" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        <button onClick={() => router.push('/dashboard/client')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 mb-6">
          <ArrowLeft className="w-4 h-4"/> {t('td.backToTasks')}
        </button>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5">
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${sc.color}`}>{t('status.' + task.status)}</span>
          </div>
          ...
          <div className="mb-5">
            <p className="text-sm font-semibold text-gray-700 mb-2">{t('td.description')}</p>
            <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap">{task.description}</p>
          </div>
          <p className="text-xs text-gray-400">{t('td.published')}: {new Date(task.created_at).toLocaleDateString('ru-RU', { year:'numeric',month:'long',day:'numeric' })}</p>
          ...
          {task.status === 'open' && (
            <div className="flex items-center gap-4 mt-4 pt-4 border-t border-gray-50">
              <button onClick={startEdit} className="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-700 font-medium">
                <Pencil className="w-3.5 h-3.5" /> {t('td.edit')}
              </button>
              ...
              <button onClick={deleteTask} className="flex items-center gap-1.5 text-xs text-red-500 hover:text-red-600 font-medium ml-auto">
                <Trash2 className="w-3.5 h-3.5" /> {t('td.delete')}
              </button>
            </div>
          )}
          {task.status !== 'open' && (
            <button onClick={deleteTask} className="flex items-center gap-1.5 text-xs text-red-500 hover:text-red-600 font-medium mt-4 pt-4 border-t border-gray-50">
              <Trash2 className="w-3.5 h-3.5" /> {t('td.deleteTask')}
            </button>
          )}
        </div>
        ...
        {task.status === 'in_progress' && task.completion_requested && !task.completion_approved && (
          <div className="bg-white rounded-2xl border border-amber-200 shadow-sm p-6 mb-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="font-semibold text-gray-900">{t('td.closeRequested')}</h2>
            </div>
            <p className="text-sm text-gray-600 mb-4">{t('td.closeRequestedDesc')}</p>
            <div className="flex gap-3">
              <button onClick={approveCompletion}
                ...
                <CheckCircle className="w-4 h-4" /> {t('td.approveClose')}
              </button>
        ...
        {task.status === 'in_progress' && task.completion_approved && (
          <div className="bg-white rounded-2xl border border-emerald-200 shadow-sm p-6 mb-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h2 className="font-semibold text-gray-900">{t('td.paymentTitle')}</h2>
            </div>
            ...
            {assignedAccountant?.kaspiQr ? (
              <div className="flex flex-col items-center bg-gray-50 rounded-2xl p-5">
                ...
                <p className="text-xs text-gray-500 mt-3">{t('td.scanKaspi')}</p>
              </div>
            ) : (
              <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
                <p className="text-sm text-amber-700">{t('td.noQr')}</p>
                <button onClick={() => router.push(`/dashboard/client?tab=messages&with=${task.accountant_id}`)}
                  className="mt-3 text-sm text-blue-600 hover:underline">{t('td.goToChat')} →</button>
              </div>
            )}
          </div>
        )}
        ...
        {qrZoom && (
          <div className="fixed inset-0 z-[60] bg-black/70 flex items-center justify-center p-4 cursor-zoom-out" onClick={() => setQrZoom(null)}>
            <div className="bg-white rounded-2xl p-4 max-w-md w-full" onClick={e => e.stopPropagation()}>
              ...
              <button onClick={() => setQrZoom(null)} className="mt-3 w-full py-2.5 bg-gray-100 hover:bg-gray-200 rounded-xl text-sm font-medium text-gray-700">{t('td.close')}</button>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={() => setEditing(false)}>
            <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 className="font-bold text-gray-900">{t('td.editTask')}</h3>
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <select value={editCategory} onChange={e => setEditCategory(e.target.value)} className={inp}>
                    {Object.keys(CATS).map(v => <option key={v} value={v}>{t('taskcat.' + v)}</option>)}
                  </select>
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">{t('td.description')}</label>
                ...
                <div className="flex gap-3 pt-2">
                  <button onClick={saveEdit} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-semibold text-sm transition-colors">{t('btn.save')}</button>
                  <button onClick={() => setEditing(false)} className="px-5 py-3 border border-gray-200 hover:bg-gray-50 text-gray-600 rounded-xl font-semibold text-sm transition-colors">{t('btn.cancel')}</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="px-6 py-5 border-b border-gray-100">
            <h2 className="font-semibold text-gray-900">{t('td.proposals')}</h2>
            <p className="text-xs text-gray-400 mt-0.5">{proposals.length} {t('td.proposalsCount')}</p>
          </div>

          {proposals.length === 0 ? (
            <div className="py-16 text-center">
              <Clock className="w-10 h-10 text-gray-200 mx-auto mb-3"/>
              <p className="text-gray-400 text-sm">{t('td.noProposals')}</p>
              <p className="text-gray-300 text-xs mt-1">{t('td.proposalsAppear')}</p>
            </div>
          ) : (
                ...
                <div key={p.id} className="p-6">
                  ...
                  {task.status === 'open' && (
                    <button onClick={() => acceptProposal(p)} disabled={accepting === p.id}
                      ...
                      {accepting === p.id ? t('td.accepting') : t('td.acceptProposal')}
                    </button>
                  )}
                  {task.status !== 'open' && task.accountant_id === p.accountant_id && (
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-100 text-emerald-700 text-sm font-semibold rounded-xl">
                        <CheckCircle className="w-4 h-4" /> {t('td.providerChosen')}
                      </span>
                  ...
                  {task.status !== 'open' && task.accountant_id !== p.accountant_id && (
                    <span className="text-xs text-gray-400">{t('td.otherChosen')}</span>
                  )}
                </div>
              ))}
            </div>
