...
// Перемешивает массив (Fisher–Yates), сохраняя одинаковую сложность для всех:
 // каждый участник проходит весь опубликованный банк, но в своём порядке.
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existing?.status === 'in_progress') {
      ...
      return NextResponse.json({
        status: 'in_progress', attemptId: existing.id, questions: ordered, answers: existing.answers || {}, manualAnswers: manual || [],
      });
    }

    const { data: pool } = await db.from('quiz_questions')
      .select('id, category, question, options, question_type')
      .eq('is_active', true);
    if (!pool || pool.length === 0) {
      return NextResponse.json({ error: 'Квиз пока не готов — вопросы ещё не опубликованы. Попробуйте позже.' }, { status: 409 });
    }

    // До явного подтверждения участника не создаём попытку.
    if (preview) return NextResponse.json({ status: 'ready', questionCount: pool.length });

    // Все активные вопросы обязательны; порядок индивидуально перемешан.
