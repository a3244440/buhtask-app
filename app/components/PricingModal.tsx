...
import { X, Check, Crown, Zap, Download } from 'lucide-react';
...
export default function PricingModal({ onClose, reason }: Props) {
  const { t } = useI18n();
  const [payPlan, setPayPlan] = useState<{ name: string; price: string } | null>(null);
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between z-10">
          <h3 className="font-bold text-gray-900">{payPlan ? t('plan.payTitle') : t('plan.choosePlan')}</h3>
          <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-400"><X className="w-5 h-5" /></button>
        </div>

        {payPlan ? (
          <div className="p-6 text-center">
            <button onClick={() => setPayPlan(null)} className="text-sm text-blue-600 hover:underline mb-4 inline-block">{t('plan.payBack')}</button>
            <div className="mb-3">
              <p className="font-bold text-gray-900 text-lg">{payPlan.name}</p>
              <p className="text-sm text-gray-500">{t('plan.payAmount')}: <b className="text-gray-900">{payPlan.price}{t('plan.perMonth')}</b></p>
            </div>
            <div className="bg-gray-50 rounded-2xl p-5 inline-block mb-4">
              <img src="/images/kaspi-qr.png" alt="Kaspi QR BuhTask" className="w-full max-w-[240px] rounded-xl mx-auto" />
            </div>
            <p className="text-sm text-gray-600 mb-3">{t('plan.payScan')}</p>
            <a href="/images/kaspi-qr.png" download="BuhTask-Kaspi-QR.png"
              ...
              <Download className="w-4 h-4" /> {t('plan.payDownload')}
            </a>
                ...
                return (
                  <div key={p.key} className={`relative rounded-2xl border ${border} p-5 flex flex-col`}>
                    ...
                    {p.current ? (
                      ...
                      <button onClick={() => setPayPlan({ name: p.name, price: p.price })}
